...
module.exports = async (context) => {
    ...
    if (!text) {
        return m.reply(
            ...
            "⚔️ *Multi-Assignment Format:* `.setvar ANTICALL_MSG = Please do not call this bot.`\n" +
            "⚔️ *Single-Value Quick-Format:* `.setvar Audio calls are blocked.` _(Automatically maps to ANTICALL_MSG)_"
        );
    }
    ...
    if (input.length === 1) {
        ...
        return m.reply(
            ...
            "🔹 `.setvar ANTICALL_MSG = System is automated.`"
        );
    }
    ...
    if (!value) {
        return m.reply(`❌ *[VALIDATION ERROR]*\n\nAssignment target payload for key *${key}* cannot evaluate to a blank string.`);
    }
