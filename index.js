...
☠️ 𝐌𝐋𝐓𝐍 𝐌𝐃 𝐀𝐍𝐓𝐈-𝐃𝐄𝐋𝐄𝐓𝐄 ☠️
✦━━━━━━━━━━━━━━✦

🩸 *𝐀𝐧𝐨𝐭𝐡𝐞𝐫 𝐭𝐚𝐫𝐠𝐞𝐭 𝐭𝐫𝐢𝐞𝐝 𝐭𝐨 𝐞𝐫𝐚𝐬𝐞 𝐭𝐡𝐞𝐢𝐫 𝐭𝐫𝐚𝐜𝐤𝐬...* 

╭─❍͙۪۫ ⋆ ❍͙۪۫─╮
🗡️ *𝐄𝐱𝐞𝐜𝐮𝐭𝐨𝐫:* ${deletedByFormatted}
🗡️ *𝐕𝐢𝐜𝐭𝐢𝐦:* ${sentByFormatted}
╰─❍͙۪۫ ⋆ ❍͙۪۫─╯

⚠️ *𝐘𝐨𝐮 𝐜𝐚𝐧𝐧𝐨𝐭 𝐡𝐢𝐝𝐞 𝐲𝐨𝐮𝐫 𝐬𝐢𝐧𝐬 𝐟𝐫𝐨𝐦 𝐭𝐡𝐞 𝐯𝐨𝐢𝐝.* 👁️‍🗨️\n\n`;
...
async function startMLTN() {
  ...
  client.ev.on("messages.upsert", async (chatUpdate) => {
    try {
      ...
      const MLTNspeed = speed() - timestamp;
      ...
      const MLTN = m.quoted || m;
      const quoted = MLTN.mtype === 'buttonsMessage' ? MLTN[Object.keys(MLTN)[1]] :
        MLTN.mtype === 'templateMessage' ? MLTN.hydratedTemplate[Object.keys(MLTN.hydratedTemplate)[1]] :
          MLTN.mtype === 'product' ? MLTN[Object.keys(MLTN)[0]] : m.quoted ? m.quoted : m;
      ...
      /*if (!IsGroup && chatbot === 'true') {
        try {
          ...
          const response = await axios.get('https://MLTN-api.vercel.app/ai/gpt', {
            params: {
              q: text
            }
          });

          const MLTN = response.data;

          if (MLTN && MLTN.status && MLTN.result) {
            await client.sendMessage(m.chat, {
              text: MLTN.result
            });
...
if (!IsGroup && voicechatbot === 'true') {
  try {
    ...
    const MLTN = response.data.result; // Access the "result" field directly

    // Generate audio URL for the response message
    const audioUrl = googleTTS.getAudioUrl(MLTN, {
...
if (!IsGroup && voicechatbot2 === 'true') {
  try {
    ...
    const audioContent = await generateMaleVoice(MLTN);
...
try {
  ...
  client.ev.on("connection.update", async (update) => {
    ...
    if (currentHour >= 5 && currentHour < 12) {
        return "𝐀𝐧𝐨𝐭𝐡𝐞𝐫 𝐦𝐨𝐫𝐧𝐢𝐧𝐠 𝐭𝐨 𝐡𝐮𝐧𝐭... 𝐓𝐡𝐞𝐢𝐫 𝐟𝐞𝐚𝐫 𝐬𝐦𝐞𝐥𝐥𝐬 𝐝𝐞𝐥𝐢𝐜𝐢𝐨𝐮𝐬 🩸";
    }
    if (currentHour >= 12 && currentHour < 18) {
        return "𝐓𝐡𝐞 𝐬𝐮𝐧 𝐢𝐬 𝐡𝐢𝐠𝐡, 𝐛𝐮𝐭 𝐦𝐲 𝐛𝐥𝐚𝐝𝐞 𝐬𝐭𝐚𝐲𝐬 𝐢𝐧 𝐭𝐡𝐞 𝐬𝐡𝐚𝐝𝐨𝐰𝐬... 🗡️";
    }
    if (currentHour >= 18 && currentHour < 22) {
        return "𝐓𝐰𝐢𝐥𝐢𝐠𝐡𝐭 𝐛𝐫𝐢𝐧𝐠𝐬 𝐭𝐡𝐞 𝐩𝐞𝐫𝐟𝐞𝐜𝐭 𝐜𝐨𝐯𝐞𝐫. 𝐓𝐢𝐦𝐞 𝐭𝐨 𝐜𝐥𝐞𝐚𝐧 𝐭𝐡𝐞 𝐬𝐭𝐫𝐞𝐞𝐭𝐬 🩸";
    }
    return "𝐓𝐡𝐞 𝐛𝐥𝐨𝐨𝐝 𝐦𝐨𝐨𝐧 𝐫𝐢𝐬𝐞𝐬. 𝐍𝐨 𝐨𝐧𝐞 𝐬𝐮𝐫𝐯𝐢𝐯𝐞𝐬 𝐭𝐡𝐢𝐬 𝐬𝐡𝐢𝐟𝐭... 💀";
};
      const message = `✦━━━━━━━━━━━━━━✦
⛧ 𝐒𝐘𝐒𝐓𝐄𝐌 𝐀𝐖𝐀𝐊𝐄𝐍𝐈𝐍𝐆 ⛧
✦━━━━━━━━━━━━━━✦
